// Problem Statemement : Find the numbers of length B and value less than C that can be made from the array A
// Time Complexity : O(N)
// Logic : the 
    ...
    int l = A.size();
        ...
        else  
        {
            ...
            else 
            {
                ...
                count = 0; // We set count as the number of elements in the first place that are smaller or equal to temp[0]
                for(int i = 0; i<l ; i++) 
                {
                    if (A[i] == 0) 
                    {
                ...
                ans += (count)*(pow(l, B-1)); // With this we are able to find the count of numbers that can be made from the with all the numbers in the array
            int flag = 0; // We set flag for the numbers that are greater than C and we need to remove
            int j=0;
